    ...
    return (
        <main className="min-h-screen w-full bg-slate-950 flex flex-col items-center py-6 md:py-10 px-4 md:px-8 overflow-y-auto" role="main">
            ...
            <div className="max-w-6xl w-full grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                <motion.section layout className="lg:col-span-2 bg-white border-[6px] border-black shadow-[12px_12px_0px_rgba(30,58,138,0.3)] p-4 md:p-10 relative z-20 rounded-sm flex flex-col min-h-[450px] md:min-h-[500px]">
                    ...
                    <div className="mt-6 md:mt-8 pt-6 border-t border-slate-100">
                        <label className="font-comic text-[10px] md:text-xs uppercase text-slate-400 block mb-4 text-center tracking-widest">Voice Your Imagination</label>
                        <div className="flex flex-wrap justify-center gap-2 md:gap-3" role="radiogroup" aria-label="Narrator Voices">
                            {voices.map(v => (
                                <button 
                                    key={v.id} 
                                    onClick={() => { onChange('narratorVoice', v.id as any); soundManager.playChoice(); }} 
                                    className={`flex flex-col items-center p-2 md:p-3 rounded-2xl border-2 transition-all w-20 md:w-28 ${input.narratorVoice === v.id ? 'border-blue-500 bg-blue-50 scale-105 shadow-md' : 'border-transparent opacity-50 hover:opacity-100 hover:bg-slate-50'}`} 
                                    role="radio" 
                                    aria-checked={input.narratorVoice === v.id}
                                    title={v.label}
                                >
                                    <span className="text-2xl md:text-3xl mb-1">{v.icon}</span>
                                    <span className="text-[8px] md:text-[10px] font-black uppercase text-center leading-none">{v.label.split(' ')[0]}</span>
                                    <span className="text-[7px] md:text-[8px] text-slate-400 mt-1 uppercase font-mono">{v.id}</span>
                                </button>
                            ))}
                        </div>
                    </div>
                    ...
                    <button 
                        ...
                        className={`comic-btn w-full mt-6 md:mt-8 py-4 md:py-5 text-xl md:text-3xl rounded-xl transition-all ${isReady && !isLoading ? 'bg-red-600 text-white shadow-[6px_6px_0px_black] active:translate-y-1' : 'bg-slate-200 text-slate-400 cursor-not-allowed shadow-none'}`} 
                ...
                <aside className="lg:col-span-1 space-y-4 md:space-y-6">
                    <section className="bg-slate-900 border-[6px] border-black p-4 md:p-6 shadow-[8px_8px_0px_black] text-white rounded-sm h-full flex flex-col min-h-[300px]" aria-labelledby="history-title">
                        <h3 id="history-title" className="font-comic text-xl md:text-2xl mb-4 md:mb-6 flex items-center gap-2">🏺 Memory Jar</h3>
                        <div className="flex-1 space-y-3 overflow-y-auto custom-scrollbar pr-1 md:pr-2 max-h-[350px] md:max-h-[450px]">
                            {history.length === 0 ? (
                                <p className="text-center text-slate-500 italic py-10 text-sm md:text-base">Your memory jar is empty. Create a story to fill it!</p>
                            ) : (
                                history.map(h => (
                                    <article key={h.id} className="group relative flex items-center bg-slate-800 p-2 md:p-3 rounded-xl border-2 border-transparent hover:border-blue-500 transition-all focus-within:ring-2 focus-within:ring-blue-500">
                                        <button 
                                        ...
                                        >
                                            <div className="w-10 h-10 bg-slate-700 rounded-full flex items-center justify-center overflow-hidden border-2 border-black flex-shrink-0">
                                                {h.avatar ? <img src={h.avatar} className="w-full h-full object-cover" alt="" /> : <span className="text-lg">📘</span>}
                                            </div>
                                            <span className="font-comic text-xs md:text-sm truncate uppercase tracking-tight">{h.story.title}</span>
                                        </button>
                                        <button 
